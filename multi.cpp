...
Base_NeTerminal * multi::derivation(int * now_lex, Scaner * table)
{
	...
	if (*myprimary->derivation(_now_lex, _All_table) == true)//Если последующее правило свернулось, то всё ок) вызываем рекурсивно полиморфный метод и определяем по крайнему левому
	{
